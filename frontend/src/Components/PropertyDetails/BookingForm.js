...
const BookingForm = ({ price,
  ...
  const { RangePicker } = DatePicker; const dispatch = useDispatch();
...
const calculatedNights = moment(dateString[1], "YYYY-MM-DD").diff(
   moment(dateString[0], "YYYY-MM-DD"), "days" );
...
const isDateDisabled = (current) => { if (!disabledDates.length)
   { return current.isBefore(Date.now(), "day");
   } else { return disabledDates.some((date) => {
    const startDate = new Date(date.start);
     const endDate = new Date(date.end).setHours(23, 59, 59, 999);
     const currentDate = new Date(current); return ( current.isBefore(Date.now(), "day") ||
